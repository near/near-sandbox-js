...
async function setSandboxConfig(homeDir, config) {
  ...
  const maxOpenFiles = config?.additionalGenesis?.["maxOpenFiles"] ?? 3e3;
  let newJsonConfig = {
    rpc: {
      limits_config: {
...
async function getDownloadPath(version) {
  const baseDir = process.env["DIR_TO_DOWNLOAD_BINARY"] ?? join2(__dirname, "..", "..", "bin");
...
var DEFAULT_NEAR_SANDBOX_VERSION = "2.9.0";
var Sandbox = class _Sandbox {
  ...
  */
  static async start(params) {
    ...
    const version = params.version || DEFAULT_NEAR_SANDBOX_VERSION;
